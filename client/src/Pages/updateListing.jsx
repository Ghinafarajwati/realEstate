...
import { faArrowUp, faTrash, faArrowUpFromBracket, faPersonShelter, faPen } from '@fortawesome/free-solid-svg-icons'
...
const UpdateListing = () => {
  ...
  const { id } = useParams()                        //idnya diambil dari app.jsx komponen update
  ...
  useEffect (() => {
    const fetchListing = async () => {
      const listing = id                  //dapatkan id urlnya
      try {
      ...
      setFormData(result)              //Ganti formDatanya!
      } catch (error) {
  ...
  return (
    <>
    <Sidebar/>
    <div className='max-w-3xl mx-auto mt-22'>
      <h1 className='text-3xl text-center font-semibold my-14'>UPDATE LISTING</h1>
      <form onSubmit={handleSubmit} className='flex flex-col sm:flex-row gap-10'>
        <div className='flex flex-col gap-4 w-80 flex-1 border p-3 pt-4 rounded-lg'>
          ...
          <input onChange={handleChange} value={formData.name} type='text' placeholder='Name' id='name' maxLength='27' minLength='10' required className='border p-2 rounded-lg'></input>
          <textarea onChange={handleChange} value={formData.description} type='text' placeholder='Description' id='description' required className='border p-2 rounded-lg resize-none h-36 overflow-y-auto placeholder-top text-gray-500'></textarea>
          <input onChange={handleChange} value={formData.address} type='text' placeholder='Address' id='address' maxLength='27' minLength='10' required className='border p-2 rounded-lg'></input>
        ...
        <div className='flex flex-wrap gap-6 justify-start'>
          ...
          <div className='flex flex-col items-center gap-2'>
            <input onChange={handleChange} value={formData.regularPrice} type="number" id="regularPrice" min='50' max='10000000' required className='p-2 w-16 border border-gray-300 rounded-lg'/> 
          ...
          {formData.offer && (                  //Kalo offer bernilai true
            <div className='flex flex-col items-center gap-2'>
            <input onChange={handleChange} value={formData.discountPrice} type="number" id="discountPrice" min='0' max='10000000' required className='p-2 w-16 border border-gray-300 rounded-lg'/> 
            <div className='flex flex-col items-center'>
            <p>Discount Price</p>
            {formData.type === 'rent' && (
              <span className='text-xs'>($ / month)</span>
        ...
        <div className='flex flex-col flex-1 gap-4'>    
          ...
          <button disabled={submitLoading || imgLoading} onSubmit={handleSubmit} className='px-5 py-3 border border-2 shadow-md border-gray-200 rounded-full capitalize bg-gradient-to-r  hover:opacity-95 disabled:opacity-80'>{submitLoading ? 'loading...' :(<div><FontAwesomeIcon icon={faPen} className='mr-2 text-sm'/>Update property</div>)}</button>
          </div>      
